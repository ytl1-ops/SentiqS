...
export default function ScheduledReportsPanel() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast(t('dashboard.reports.scheduled.created'));
      setShowCreate(false);
      resetForm();
      fetchSchedules();
    } catch (err: unknown) {
  ...
  const handleToggle = async (sch: ScheduledReport) => {
    try {
      ...
      showToast(t('dashboard.reports.scheduled.toggled'));
      fetchSchedules();
    } catch (err: unknown) {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      showToast(t('dashboard.reports.scheduled.deleted'));
      fetchSchedules();
    } catch (err: unknown) {
  ...
  const handleTriggerNow = async (sch: ScheduledReport) => {
    try {
      ...
      showToast(`"${sch.name}" — ${t('dashboard.reports.scheduled.toast.triggered')}`);
      fetchSchedules();
    } catch (err: unknown) {
    ...
    return (
      <div className="space-y-5 pt-4 border-t border-gray-100">
        <div className="py-12 text-center bg-white rounded-lg border border-gray-100">
          ...
          <p className="text-xs text-sentiqs-gray-text mb-3">{t('dashboard.reports.scheduled.loadError')}</p>
          <button
            type="button"
            onClick={fetchSchedules}
            className="px-4 py-2 text-xs font-semibold text-white bg-sentiqs-navy rounded-lg hover:bg-sentiqs-navy/90 transition-colors whitespace-nowrap"
          >
            {t('dashboard.reports.scheduled.retry')}
          </button>
        </div>
      </div>
    );
