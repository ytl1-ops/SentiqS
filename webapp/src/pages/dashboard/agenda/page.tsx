...
const priorityBadge = (p: string) => {
  ...
  return `px-1.5 py-0.5 rounded text-[9px] font-semibold border ${map[p] || ''}`;
};
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-sentiqs-gray-text mt-0.5">
            {agendaEvents.length} événements programmés — {upstreamEvents.length} à venir
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-semibold text-sentiqs-navy uppercase tracking-wider flex items-center gap-2">
            ...
            {selectedDate ? `Événements du ${selectedDate.split('-').reverse().join('/')}` : 'Prochains événements'}
          </h3>

          {selectedDate && selectedDateEvents ? (
            selectedDateEvents.length === 0 ? (
              <div className="bg-white rounded-lg border border-gray-100 p-6 text-center">
                <div className="w-10 h-10 mx-auto rounded-full bg-gray-50 flex items-center justify-center mb-2">
                  <i className="ri-calendar-line text-sentiqs-gray-text text-lg" />
                </div>
                <p className="text-xs text-sentiqs-gray-text">Aucun événement ce jour</p>
              </div>
            ) : (
                    ...
                    >
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold text-sentiqs-navy leading-snug line-clamp-2">{ev.title}</p>
                          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                            <span className={`px-1.5 py-0.5 rounded text-[9px] font-semibold border ${cfg.bg} ${cfg.color}`}>
                              {isFr ? cfg.label : cfg.labelEn}
                            </span>
                            <span className={priorityBadge(ev.priority)}>{t(`dashboard.severity.${ev.priority}`)}</span>
                          </div>
                          ...
                          {selectedEvent === ev.id && (
                            <div className="mt-3 pt-3 border-t border-gray-100 space-y-2">
                              <div className="flex items-center justify-between text-[10px]">
                                <span className="text-sentiqs-gray-text">Durée</span>
                                <span className="font-semibold text-sentiqs-navy">{ev.duration}</span>
                              </div>
                              <div className="flex items-center justify-between text-[10px]">
                                <span className="text-sentiqs-gray-text">Lieu</span>
                                <span className="font-semibold text-sentiqs-navy text-right max-w-[160px]">{ev.location}</span>
                              </div>
                              <div className="flex items-center justify-between text-[10px]">
                                <span className="text-sentiqs-gray-text">Organisateur</span>
                                <span className="font-semibold text-sentiqs-navy">{ev.organizer}</span>
                              </div>
                              <div className="flex items-center justify-between text-[10px]">
                                <span className="text-sentiqs-gray-text">Participants</span>
                                <span className="font-semibold text-sentiqs-navy">{ev.participants}</span>
                              </div>
                              <p className="text-[10px] text-sentiqs-gray-text leading-relaxed pt-1">{ev.description}</p>
                              <div className="flex gap-2 pt-1">
                                <button type="button" className="text-[10px] font-semibold text-sentiqs-navy hover:underline flex items-center gap-1">
                                  <i className="ri-calendar-check-line text-xs" /> Confirmer présence
                                </button>
                                <button type="button" onClick={() => { setShareTarget({ id: ev.id, title: ev.title }); setShareModalOpen(true); }} className="text-[10px] font-semibold text-sentiqs-navy hover:underline flex items-center gap-1">
            ...
            <div className="space-y-2">
              {upstreamEvents.slice(0, 8).map((ev) => {
                const cfg = typeConfig[ev.type as EventType];
                ...
                return (
                  <button
                    ...
                    className="w-full bg-white rounded-lg border border-gray-100 p-3 text-left hover:border-gray-200 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-sentiqs-navy leading-snug line-clamp-1">{ev.title}</p>
              ...
              {upstreamEvents.length > 8 && (
                <p className="text-center text-[10px] text-sentiqs-gray-text pt-1">
                  + {upstreamEvents.length - 8} autres événements...
                </p>
              )}
            </div>
          )}
